-- =============================================================
-- Liftly Migration v2
-- Safe to run on existing DB — uses ALTER TABLE for posts,
...
-- =============================================================
-- MIGRATE EXISTING POSTS TABLE (add missing columns)
-- =============================================================
alter table public.posts
  ...
  add column if not exists gradient text default 'ocean';

-- Enable RLS on posts (safe if already enabled)
alter table public.posts enable row level security;

-- Drop old permissive policies if they exist, then recreate
...
drop policy if exists "Invite codes readable by all" on public.invite_codes;
drop policy if exists "Invite codes insertable by all" on public.invite_codes;
drop policy if exists "Invite codes updatable by all" on public.invite_codes;
...
-- =============================================================
-- REACTIONS TABLE (replaces likes + saved_posts)
-- =============================================================
...
drop policy if exists "Challenges viewable by authenticated users" on public.daily_challenges;
drop policy if exists "Challenges insertable by anyone" on public.daily_challenges;
...
drop policy if exists "Completions viewable by authenticated users" on public.challenge_completions;
drop policy if exists "Users can manage own completions" on public.challenge_completions;
...
drop policy if exists "Users can view own journal" on public.impact_journal;
drop policy if exists "Users can insert own journal entries" on public.impact_journal;
...
-- Done! Run schema_v2_additions.sql next for badges, early testers, etc.
